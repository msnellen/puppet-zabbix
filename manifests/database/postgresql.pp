  ...
  case $zabbix_type {
    ...
    default: {
      if versioncmp($zabbix_version, '6.0') >= 0 {
        $zabbix_server_create_sql = "cd ${schema_path} && if [ -f server.sql.gz ]; then gunzip -f server.sql.gz ; fi && psql -h '${database_host}' -U '${database_user}' ${port}-d '${database_name}' -f server.sql && touch /etc/zabbix/.schema.done"
      } else {
